...
// Grid code based on: https://asliceofrendering.com/scene%20helper/2020/01/05/InfiniteGrid/
...
vec4 grid(vec3 fragPos3D, float scale, bool drawAxis) 
{
    ...
    vec2 grid = abs(fract(coord - 10.0) - 0.5) / derivative;
...
void main()
{
	...
	vec3 v = normalize(ubo.eyePos - inWorldPos);    // Vector from surface point to camera
	...
	// xz guides
    vec4 guideColor = (grid(inWorldPos, 10, true) + grid(inWorldPos, 1, true)); 
    
	// grid
	float gridSize = 0.0;   
	float edgeWidth = 0.0; 
	float tileLuminance = 1.0;
	float edgeContrast = 0.5;

	vec2 uv = inWorldPos.xy;
	uv = (mod(uv, gridSize) - mod(uv - edgeWidth, gridSize) - edgeWidth) * 1.0/gridSize;
	outColor *= vec4(tileLuminance - length(inWorldPos) * edgeContrast * guideColor );
}
